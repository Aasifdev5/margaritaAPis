                                    ...
                                    <div class="row">
                                        <div class="col-lg-8">
                                            <div class="ticket-replies mb-4">
                                                <h5 class="mb-3 ">{{ __('Ticket Replies') }}</h5>

                                                @forelse($ticketMessages as $ticketMessage)
                                                <div class="mb-3 p-3 {{ $ticketMessage->reply_admin_user_id ? ' border-left border-primary' : '' }} rounded">
                                                    ...
                                                    <p class="">{{ $ticketMessage->message }}</p>
                                            ...
                                            @empty
                                                <p class="">{{ __('No Reply Found') }}</p>
                                            @endforelse

                                            </div>

                                            <!-- Reply Form -->
                                            <div class="reply-form  p-4 rounded shadow-sm">
                                                <h5 class="mb-3 ">{{ __('Write a reply') }}</h5>
                                                <form action="{{ route('support-ticket.messageStore') }}" method="post"
                                                    ...
                                                    <div class="form-group mb-3">
                                                        <textarea class="form-control " name="message" rows="5" placeholder="{{ __('Write your message') }}"
                                                            required></textarea>
                                                        @error('message')
                                                    ...
                                                    <div class="form-group mb-3">
                                                        <label
                                                            class="font-weight-bold ">{{ __('Upload Your File') }}</label>
                                        ...
                                        <!-- Ticket Info Sidebar -->
                                        <div class="col-lg-4">
                                            <div class="ticket-info  p-4 rounded shadow-sm">
                                                <h5 class="mb-3 ">{{ __('Ticket Info') }}</h5>
                                                <p class=""><strong >{{ __('Status') }}:</strong>
                                                    <span
                                                        class="badge {{ $ticket->status == 1 ? 'badge-success' : 'badge-danger' }}">
                                                        {{ $ticket->status == 1 ? __('Open') : __('Closed') }}
                                                    </span>
                                                </p>
                                                <p class=""><strong >{{ __('Subject') }}:</strong>
                                                    #{{ $ticket->ticket_number }} - {{ $ticket->subject }}</p>
                                                @if ($ticket->department)
                                                    <p class=""><strong >{{ __('Department') }}:</strong>
                                                        {{ $ticket->department->name }}</p>
                                                @endif
                                                @if ($ticket->priority)
                                                    <p class=""><strong >{{ __('Priority') }}:</strong>
                                                        {{ $ticket->priority->name }}</p>
                                                @endif
                                                @if ($ticket->service)
                                                    <p class=""><strong >{{ __('Related Service') }}:</strong>
                                                        {{ $ticket->service->name }}</p>
                                                @endif
                                                <p class=""><strong >{{ __('Opened') }}:</strong>
                                                    {{ $ticket->created_at->format('d M, Y h:i A') }}</p>
                                                <p class=""><strong >{{ __('Last Response') }}:</strong>
