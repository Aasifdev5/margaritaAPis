...
{
    if (!empty($key)) {
        return Setting::selectSetting($key);
    }
    $Setting = Setting::pluck('value', 'key')->all();
    return array_to_object($Setting);
}
...
function dateFormat($date)
{
    ...
    $format = Date::parse($date)->format(Setting::dateFormats()[Setting('general')->date_format]);
    return $format;
}
...
{
    $name = Setting('general')->site_name;
    $title = null;
    $section = null;
